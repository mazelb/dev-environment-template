...
def semantic_search(query: str, limit: int = 5) -> dict:
    ...
    response = requests.post(
        f"{BASE_URL}/search", json={"query": query, "limit": limit}
    )
    response.raise_for_status()
...
def rag_query(query: str, k: int = 3) -> dict:
    ...
    response = requests.post(f"{BASE_URL}/rag/query", json={"query": query, "k": k})
    response.raise_for_status()
...
def agent_query(query: str) -> dict:
    ...
    response = requests.post(f"{BASE_URL}/agents/query", json={"query": query})
    response.raise_for_status()
